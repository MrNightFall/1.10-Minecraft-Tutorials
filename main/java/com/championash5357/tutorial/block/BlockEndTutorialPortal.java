...
import com.championash5357.tutorial.world.TutorialEndTeleporter;
import com.championash5357.tutorial.world.dimension.TutorialDimensionRegistry;

public class BlockEndTutorialPortal extends BlockBreakable{
	...
	public BlockEndTutorialPortal() {
		super(Material.PORTAL, false);
		this.setUnlocalizedName("tutorial_end_portal_block");
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if(worldIn.provider.isSurfaceWorld() && worldIn.getGameRules().getBoolean("doMobSpawning") && rand.nextInt(2000) < worldIn.getDifficulty().getDifficultyId()) {
			...
			if(i > 0 && !worldIn.getBlockState(blockpos.up()).isNormalCube()) {
				Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.getEntityStringFromClass(EntityFourArms.class), (double)blockpos.getX() + 0.5d, (double)blockpos.getY() + 1.1f, (double)blockpos.getZ() + 0.5d);
				if(entity != null) {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockEndTutorialPortal.Size blockportal$size = new BlockEndTutorialPortal.Size(worldIn, pos, EnumFacing.Axis.X);
		...
		if(blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			...
			BlockEndTutorialPortal.Size blockportal$size1 = new BlockEndTutorialPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn) {
		...
		if(enumfacing$axis == EnumFacing.Axis.X) {
			...
			BlockEndTutorialPortal.Size blockportal$size = new BlockEndTutorialPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if(!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && entityIn instanceof EntityPlayerMP) {
			...
			} else if (thePlayer.dimension != TutorialDimensionRegistry.tutorialEndDimensionID) {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, TutorialDimensionRegistry.tutorialEndDimensionID, new TutorialEndTeleporter(thePlayer.mcServer.worldServerForDimension(TutorialDimensionRegistry.tutorialEndDimensionID)));
			} else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 0, new TutorialEndTeleporter(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	...
	public static class Size
    {
        ...
        protected int calculatePortalHeight()
        {
            ...
            for (this.height = 0; this.height < 21; ++this.height)
            {
                for (int i = 0; i < this.width; ++i)
                {
                    ...
                    if (block == TutorialBlocks.tutorial_end_portal_block)
                    {
        ...
        {
            return blockIn.getMaterial(blockIn.getDefaultState()) == Material.AIR || blockIn == Blocks.FIRE || blockIn == TutorialBlocks.tutorial_end_portal_block;
        }
        ...
        public void placePortalBlocks()
        {
            for (int i = 0; i < this.width; ++i)
            {
                ...
                {
                    this.world.setBlockState(blockpos.up(j), TutorialBlocks.tutorial_end_portal_block.getDefaultState().withProperty(BlockPortal.AXIS, this.axis), 2);
                }
            }
        }
    }
