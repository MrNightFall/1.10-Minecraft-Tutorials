...
public class TutorialEndChunkProvider implements IChunkGenerator
{
    private final Random rand;
    protected static final IBlockState COMMAND_BLOCK = Blocks.COMMAND_BLOCK.getDefaultState();
    ...
    double[] ar;
    double[] br;
    private final WorldGenEndTutorialIsland endIslands = new WorldGenEndTutorialIsland();
    private int chunkX = 0;
    private int chunkZ = 0;

    public TutorialEndChunkProvider(World worldObjIn, long seed)
    {
        ...
        ctx = TerrainGen.getModdedNoiseGenerators(worldObjIn, this.rand, ctx);
    ...
    public void buildSurfaces(ChunkPrimer primer)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            for (int j = 0; j < 16; ++j)
            {
                int k = 1;
                int l = -1;
                IBlockState iblockstate = COMMAND_BLOCK;
                IBlockState iblockstate1 = COMMAND_BLOCK;
    ...
    public void populate(int x, int z)
    {
        ...
        int j = x * 16;
        int k = z * 16;
        BlockPos blockpos = new BlockPos(j, 0, k);
        Biome biome = this.worldObj.getBiomeGenForCoords(blockpos.add(16, 0, 16));
        
        new TutorialDimensionHouse().generate(worldObj, rand, new BlockPos(j, 0, k));

        biome.decorate(this.worldObj, this.worldObj.rand, blockpos);
