...
import com.championash5357.tutorial.world.TutorialDimensionHouse;

public class TutorialChunkProvider implements IChunkGenerator {
	
	protected static final IBlockState AIR = Blocks.AIR.getDefaultState();
	protected static final IBlockState PURPLE_GLOWSTONE = TutorialBlocks.purple_glowstone.getDefaultState();
	protected static final IBlockState BEDROCK = Blocks.BEDROCK.getDefaultState();
	protected static final IBlockState ACID = TutorialFluids.BlockAcid.instance.getDefaultState();
	protected static final IBlockState GLOWSTONE = Blocks.GLOWSTONE.getDefaultState();
	protected static final IBlockState DIAMOND_BLOCK = Blocks.DIAMOND_BLOCK.getDefaultState();
    ...
    private final WorldGenerator goldGen = new WorldGenMinable(Blocks.GOLD_ORE.getDefaultState(), 14, BlockMatcher.forBlock(TutorialBlocks.purple_glowstone));
    private final WorldGenHellLava acidTrapGen = new WorldGenHellLava(TutorialFluids.BlockAcid.instance, true);
    private final WorldGenHellLava hellSpringGen = new WorldGenHellLava(TutorialFluids.BlockAcid.instance, false);
    ...
    public TutorialChunkProvider(World worldIn, long seed)
    {
        this.world = worldIn;
    ...
    public void buildSurfaces(int p_185937_1_, int p_185937_2_, ChunkPrimer primer)
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            for (int k = 0; k < 16; ++k)
            {
                ...
                int i1 = -1;
                IBlockState iblockstate = PURPLE_GLOWSTONE;
                IBlockState iblockstate1 = PURPLE_GLOWSTONE;
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    if (j1 < 127 - this.rand.nextInt(5) && j1 > this.rand.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == TutorialBlocks.purple_glowstone)
                            {
                                if (i1 == -1)
                                {
                                    if (l <= 0)
                                    ...
                                    else if (j1 >= i - 4 && j1 <= i + 1)
                                    {
                                        ...
                                        if (flag1)
                                        {
                                            iblockstate = GLOWSTONE;
                                            iblockstate1 = PURPLE_GLOWSTONE;
                                        }

                                        if (flag)
                                        {
                                            iblockstate = DIAMOND_BLOCK;
                                            iblockstate1 = DIAMOND_BLOCK;
                                        }
                                    }
	...
	public void populate(int x, int z)
    {
        ...
        if (net.minecraftforge.event.terraingen.TerrainGen.generateOre(this.world, this.rand, goldGen, blockpos, net.minecraftforge.event.terraingen.OreGenEvent.GenerateMinable.EventType.QUARTZ))
        for (int l1 = 0; l1 < 16; ++l1)
        {
        ...
        new TutorialDimensionHouse().generate(world, rand, new BlockPos(i, 0, j));
